...
function makeUnetDCGAN()
  ...
  -- ngf*8 x 4 x 4 
  local enc5 = nn.Tanh()(SpatialBatchNormalization(opt.contentDim)(SpatialConvolution(ngf*8, opt.contentDim, 4, 4)(enc4)))
  -- contentDim x 1 x 1

  local join = nn.JoinTable(2)({enc5, pose})
  ...
  -- ngf*8 x 4 x 4
  local dec2 = nn.LeakyReLU(0.2, true)(SpatialBatchNormalization(ngf*4)(SpatialFullConvolution(ngf*8*2, ngf*4, 4, 4, 2, 2, 1, 1)(nn.JoinTable(2)({dec1, enc4} ))))
  -- ngf*4 x 8 x 8
  local dec3 = nn.LeakyReLU(0.2, true)(SpatialBatchNormalization(ngf*2)(SpatialFullConvolution(ngf*4*2, ngf*2, 4, 4, 2, 2, 1, 1)( nn.JoinTable(2)({dec2, enc3} ))))
  -- ngf*2 x 16 x 16
  local dec4 = nn.LeakyReLU(0.2, true)(SpatialBatchNormalization(ngf)(SpatialFullConvolution(ngf*2*2, ngf, 4, 4, 2, 2, 1, 1)( nn.JoinTable(2)({dec3, enc2} ))))
  -- ngf x 32 x 32
  local dec5 = nn.Sigmoid()(SpatialFullConvolution(ngf*2, nc, 4, 4, 2, 2, 1, 1)( nn.JoinTable(2)({dec4, enc1})))
  -- nc x 64 x 64
  local net = nn.gModule({x, pose}, {dec5, enc5})
  initModel(net)
  return net
end
...
function makeUnetVGG()
  ...
  -- 2 -> 1
  local content = nn.Tanh()(SpatialBatchNormalization(opt.contentDim)(SpatialConvolution(512, opt.contentDim, 2, 2)(mp_5)))

  local join = nn.JoinTable(2)({content, pose})
  
  local comb = nn.LeakyReLU(0.2, true)(SpatialBatchNormalization(512)(SpatialFullConvolution(opt.contentDim+opt.poseDim, 512, 2, 2)(join)))
