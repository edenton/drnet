...
function makeContentEncoder()
  ...
  add_dcgan_layer(net, nc*opt.nShare, ngf) -- 32 -> 16
  add_dcgan_layer(net, ngf, ngf*2) -- 16 -> 8
  add_dcgan_layer(net, ngf*2, ngf*4) -- 8 -> 4
  add_dcgan_layer(net, ngf*4, ngf*8) -- 4 -> 2
...
function makePoseEncoder()
  local net = nn.Sequential()
  add_dcgan_layer(net, nc, ngf) -- 32 -> 16
  add_dcgan_layer(net, ngf, ngf*2) -- 16 -> 8
  add_dcgan_layer(net, ngf*2, ngf*4) -- 8 -> 4
  add_dcgan_layer(net, ngf*4, ngf*8) -- 4 -> 2
...
function makeDecoder()
  local net = nn.Sequential()
  net:add(nn.JoinTable(2))
  net:add(SpatialFullConvolution(opt.poseDim+opt.contentDim, ngf*8, 4, 4))
